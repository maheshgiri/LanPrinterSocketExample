...
public class Pos {
    ...
    public byte[] POS_S_TextOut(String pszString, String encoding, int nOrgx, int nWidthTimes, int nHeightTimes, int nFontType, int nFontStyle) {
        ...
        if (!(nOrgx > '\uffff' | nOrgx < 0 | nWidthTimes > 7 | nWidthTimes < 0 | nHeightTimes > 7 | nHeightTimes < 0 | nFontType < 0 | nFontType > 4 | pszString.length() == 0)) {
            ...
            data = DataUtils.byteArraysToBytes(new byte[][]{this.Cmd.ESC_dollors_nL_nH, this.Cmd.GS_exclamationmark_n, tmp_ESC_M_n, this.Cmd.GS_E_n, this.Cmd.ESC_line_n, this.Cmd.FS_line_n, this.Cmd.ESC_lbracket_n, this.Cmd.GS_B_n, this.Cmd.ESC_V_n, pbString1});
   ...
   /* public void POS_SetKey(byte[] key) {
    ...
    public boolean POS_QueryStatus(byte[] precbuf, int timeout) {
        byte[] pcmdbuf = new byte[]{29, 114, 1};
    ...
    public boolean POS_QueryOnline(int timeout) {
        byte[] pcmdbuf = new byte[]{29, 114, 1, 16, 4, 1};
    ...
    public boolean EMBEDDED_WriteToUart(byte[] buffer, int offset, int count) {
        byte[] header = new byte[]{31, 82, (byte)(count >> 8 & 255), (byte)(count & 255)};
